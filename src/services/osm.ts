...
function createOverpassQuery(
    ...
    amenityTypes.forEach((amenityType) => {
        ...
        if (amenityQuery) {
            ...
            queryParts.push(
                `node[${amenityQuery.osmQuery}](${south},${west},${north},${east});`,
            );
            queryParts.push(
                `way[${amenityQuery.osmQuery}](${south},${west},${north},${east});`,
            );
        }
    });
...
function convertOSMElementToAmenity(element: OSMElement): OSMAmenity | null {
    ...
    const name =
        tags.name || tags['name:en'] || tags.description || defaultName;
    ...
    if (tags.opening_hours)
        descriptionParts.push(`Hours: ${tags.opening_hours}`);
...
export async function fetchOSMWaterFountains(
    bounds = BRIGHTON_BOUNDS,
): Promise<OSMAmenity[]> {
